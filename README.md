...
```bash
git clone https://github.com/BAAI-EI-DATA/robodriver-robot-boostert1-aio-ros2.git && cd robodriver-robot-boostert1-aio-ros2
```
...
python robodriver/scripts/run.py \
  --robot.type=BoosterT1_aio_ros2
```

> If your registered robot type name is different, replace `BoosterT1_aio_ros2` with the actual type shown in your environment.
...
```text
robodriver-robot-booster-aio-ros2/
├── lerobot_robot_boostert1-aio_ros2/        # optional related dataset/adapter code
├── robodriver_robot_boostert1_aio_ros2/     # RoboDriver plugin package
├── README.md
└── pyproject.toml
```
